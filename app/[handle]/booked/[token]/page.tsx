      ...
      <div className="w-full max-w-md animate-fade-in">
        <div className="card overflow-hidden">
          ...
          <div className="space-y-4 p-6">
            ...
            <Row
              label="Deposit paid"
              value={booking.depositStatus === "paid" ? gbp(booking.depositPennies) : "—"}
            />
            <Row
              label="Balance due on the day"
              value={gbp(booking.balancePennies)}
              strong
            />

            {booking.balancePennies > 0 && booking.balanceStatus !== "paid" && (
              <Link
                href={`/pay/${booking.balanceToken}`}
            ...
            <Link
              href={`/${tech.handle}`}
              className="flex w-full items-center justify-center gap-2 rounded-xl border border-black/10 py-3 text-sm font-medium text-ink-soft hover:bg-black/[0.03]"
            >
