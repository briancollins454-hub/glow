...
import { listCategories, listServices } from "@/lib/db/repo";
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-ink-soft">
            Set prices, deposits, patch-test rules and infill windows.
          </p>
        </div>
      </div>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <FolderPlus className="h-5 w-5 text-brand-600" /> Add a category
            </CardTitle>
            <CardDescription>
              Categories hold patch-test defaults (e.g. Lashes, Brows).
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form action={addCategoryAction} className="grid gap-3 sm:grid-cols-2">
              <div className="sm:col-span-2">
                ...
                <Input name="name" placeholder="Lashes" required />
              </div>
              <div>
                <Label>Patch test valid for (days)</Label>
                <Input name="validityDays" type="number" defaultValue={180} />
              </div>
              <div>
                <Label>Min lead before appt (hours)</Label>
                <Input name="minLeadHours" type="number" defaultValue={24} />
              </div>
              <div className="sm:col-span-2">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {categories.map((c) => (
                  <Badge key={c.id} tone="brand">
                    {c.name} · {c.patchTestValidityDays}d
                  </Badge>
                ))}
              </div>
            )}
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Plus className="h-5 w-5 text-brand-600" /> Add a service
            </CardTitle>
            <CardDescription>
              {categories.length === 0
                ? "Add a category first."
                : "New services appear on your booking page immediately."}
            </CardDescription>
          </CardHeader>
          <CardContent>
            {categories.length === 0 ? (
              <p className="text-sm text-ink-faint">
                You need at least one category before adding services.
              </p>
            ) : (
                ...
                <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3">
                  <div className="min-w-0">
                    ...
                    <p className="mt-0.5 text-xs text-ink-faint">
                      {category?.name} · {minutesToLabel(s.durationMin)} · {gbp(s.pricePennies)} · {depositFor(s) > 0 ? `${gbp(depositFor(s))} deposit` : "no deposit"}
                    </p>
                  </div>
